...
/*
function partsSums(ls) {
  let sum = ls.reduce((acc, v) => acc + v, 0)
...
//OPTION 3 - Works slowly on very large data sets
// const partsSums = ls => ls.reduceRight((res, cur) => (res.unshift(cur += res[0]), res), [0]);



//OPTION 4 - Works slowly on very large data sets
/*
function partsSums(ls) {
  let res = [0];

  ls.reverse().forEach(v => res.unshift(v += res[0]));
