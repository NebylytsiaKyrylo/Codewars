...
function validSolution(board) {
  const trueLine = '123456789';

  //Check horizontal arrays for validity
  const horizontal = board.every(line => line.slice().sort().join('') == trueLine);

  //Make vertical arrays and check for validity
  let vertical = [];
  
  for (let i = 0; i < board.length; i++) {
    let verticalLine = [];
    board.forEach(v => verticalLine.push(v[i]));
    vertical.push(verticalLine);
  }

  vertical = vertical.every(line => line.sort().join('') == trueLine);

  //sub-grids validation
  let subGrids = [];

  for (let i = 0, k = 3; k < board.length; i += 3, k += 3) {
    let subGrid = [];

    board.forEach(line => {
      subGrid.push(...line.slice(i, k));
      if (subGrid.length == 9) {
  ...
  subGrids = subGrids.every(line => line.sort().join('') == trueLine);

  return horizontal && vertical && subGrids;
}
